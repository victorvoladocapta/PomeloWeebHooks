...
public sealed class PomeloStatementOpenedStore(WebhookDbContext db) : IPomeloStatementOpenedStore
{
    public async Task<bool> TryAddAsync(PomeloStatementOpenedEvent entity, CancellationToken ct)
    {
        if (await db.PomeloStatementOpenedEvents.AsNoTracking().AnyAsync(x => x.IdempotencyKey == entity.IdempotencyKey, ct))
            return false;
        db.PomeloStatementOpenedEvents.Add(entity);
