...
public sealed class PomeloRevertedOperationStore(WebhookDbContext db) : IPomeloRevertedOperationStore
{
    public async Task<bool> TryAddAsync(PomeloRevertedOperationEvent entity, CancellationToken ct)
    {
        if (await db.PomeloRevertedOperationEvents.AsNoTracking().AnyAsync(x => x.IdempotencyKey == entity.IdempotencyKey, ct)) return false;
        db.PomeloRevertedOperationEvents.Add(entity);
